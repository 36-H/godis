...
// Open 开启一个数据库实例
func Open(dirPath string) (*Godis, error) {
	// 如果数据库目录不存在，则新建一个
...
// Put 写入数据
func (db *Godis) Put(key []byte, value []byte) (err error) {
	if len(key) == 0 {
		return
	}
...
func (db *Godis) exist(key []byte) (int64, error) {
	// 从内存当中取出索引信息
...
// Get 取出数据
func (db *Godis) Get(key []byte) (val []byte, err error) {
	if len(key) == 0 {
		return
	}
...
// Del 删除数据
func (db *Godis) Del(key []byte) (err error) {
	if len(key) == 0 {
		return
	}
...
// 从文件当中加载索引
func (db *Godis) loadIndexesFromFile() {
	if db.dbFile == nil {
		return
	}
